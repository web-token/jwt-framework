...
final class JWELoaderTest extends EncryptionTest
{
    ...
    {
        $this->checkCheckerComponentIsAvailable();
        $key = JWK::create([
            'kty' => 'oct',
    ...
    private function checkCheckerComponentIsAvailable()
    {
        if (!class_exists(HeaderCheckerManager::class)) {
            $this->markTestSkipped('Checker Component not installed.');
        }
    }
}
